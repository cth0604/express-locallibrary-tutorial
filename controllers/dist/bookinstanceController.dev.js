...
var async = require("async"); // Display list of all BookInstances.
...
exports.bookinstance_create_post = [// Validate and sanitise fields.
body("book", "Book must be specified").trim().isLength({
  min: 1
}).escape(), body("imprint", "Imprint must be specified").trim().isLength({
  min: 1
}).escape(), body("status").escape(), body("due_back", "Invalid date").optional({
  checkFalsy: true
...
exports.bookinstance_update_post = [// Validate and sanitise fields.
body("book", "Book must be specified").trim().isLength({
  min: 1
}).escape(), body("imprint", "Imprint must be specified").trim().isLength({
  min: 1
